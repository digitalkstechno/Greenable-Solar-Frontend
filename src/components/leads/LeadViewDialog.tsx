...
export default function LeadViewDialog({ lead, statuses, currentUser, onClose, onRefresh }: Props) {
  ...
  const [attachmentDeleteConfirm, setAttachmentDeleteConfirm] = useState<any>(null);
  ...
  const executeDeleteAttachment = async () => {
    if (!attachmentDeleteConfirm) return;
    setDeletingAttachment(true);
    try {
      await axios.delete(
        `${baseUrl.getBaseUrl?.endsWith('/') ? baseUrl.getBaseUrl.slice(0, -1) : baseUrl.getBaseUrl}/lead/${lead?._id}/attachments/${attachmentDeleteConfirm._id}`,
        { headers: { Authorization: `Bearer ${getAuthToken()}` } }
      );
      setLocalAttachments(prev => prev.filter(a => a._id !== attachmentDeleteConfirm._id));
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        setDepartments(prev => prev.length === 0 ? depts : prev);
  ...
  const handleReassign = async (userId: string) => {
    ...
    try {
      ...
      if (res.data?.data?.assignedTo) {
        ...
        const newAssignee = reassignUsers.find(u => u._id === userId);
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-3 p-2 bg-white rounded-lg border border-gray-200 hover:border-gray-300 transition-all">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() => setAttachmentDeleteConfirm(att)}
                            className="p-1.5 hover:bg-gray-100 rounded transition-colors text-gray-500 hover:text-gray-700"
                            title="Delete"
                          >
                  ...
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="bg-gray-100 border-b border-gray-200">
                        ...
                        <th className="sticky top-0 z-10 bg-gray-100 px-4 py-3 font-semibold text-gray-600 text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
      ...
      {/* Attachment Delete Confirmation Popup */}
      {
        attachmentDeleteConfirm && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center">
            <div
              className="absolute inset-0 bg-black/40 backdrop-blur-sm"
              onClick={() => !deletingAttachment && setAttachmentDeleteConfirm(null)}
            />
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 flex flex-col items-center gap-4" style={{ animation: 'fadeInScale 0.2s ease' }}>
              <div className="flex items-center justify-center w-14 h-14 rounded-full bg-[#f9e9d9]">
                <Trash2 className="h-7 w-7 text-[#d87612]" />
              ...
              <div className="text-center">
                <h3 className="text-base font-bold text-gray-800 mb-1">Delete Attachment?</h3>
                <p className="text-sm text-gray-500">
                  Are you sure you want to delete the attachment{' '}
                  <span className="font-semibold text-gray-800">{attachmentDeleteConfirm.originalName}</span>?
                </p>
              </div>
              <div className="flex gap-3 w-full mt-1">
                <button
